...
def prov(state_type: StateType = StateType.OBSERVED) -> Provenance:
    return Provenance(source="fixture", dataset="fixture", source_url="https://example.test", state_type=state_type)
...
def test_snapshot_uses_latest_per_station_without_interpolation() -> None:
    ...
    snap = compose_snapshot(timestamp=now, stations=[station("1"), station("2")], observations=observations, official_areas=[])
...
def test_snapshot_marks_stale_observations() -> None:
    now = datetime(2026, 9, 14, 12, tzinfo=UTC)
    snap = compose_snapshot(timestamp=now, stations=[station("1")], observations=[obs("1", now - timedelta(hours=4), 25)], official_areas=[])
...
def test_scenario_is_labelled_and_does_not_claim_prediction() -> None:
    now = datetime(2026, 9, 14, 12, tzinfo=UTC)
    snap = compose_snapshot(timestamp=now, stations=[station("1")], observations=[obs("1", now, 25)], official_areas=[])
